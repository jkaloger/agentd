...
Symphony gives each issue a plain per-issue directory under a workspace root, reused across
runs, populated via hooks (SPEC §9). agentd is git-like, and lazyspec iterations map naturally
onto a branch → PR flow. Options: git worktree per ticket, full clone per ticket, or Symphony's
plain dir.
...
Symphony's safety invariants still hold (SPEC §9.5): agent cwd is the worktree, the path must
...
- Worktree lifecycle (`git worktree add` / `remove`) replaces Symphony's dir create/cleanup;
