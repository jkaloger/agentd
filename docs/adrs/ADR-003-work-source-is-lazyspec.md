...
Symphony reads work from Linear over GraphQL (SPEC §11). agentd's tracker is **lazyspec** —
...
contradicts Symphony §11.5 (agent performs tracker writes).

## Decision

We will consume lazyspec by **shelling out to `lazyspec … --json` behind a `Tracker` trait**
(the Symphony adapter seam, preserved so Linear or others remain future adapters).
...
Symphony's configurable `active_states` / `terminal_states`. The shipped default is the
...
- Diverges from Symphony §11.5 — the daemon, not the agent, writes ticket state; the agent
