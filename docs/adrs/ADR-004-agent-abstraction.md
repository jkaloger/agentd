...
SPEC §10 hardwires the Codex app-server protocol (thread/turn, streaming JSON over stdio,
...
loops to the goal internally, so the SPEC's multi-turn app-server loop largely collapses to
...
- codex reuses the SPEC's design almost directly; opencode wraps its server API.
